//! REQUIREMENT: VAL-004 — Missing coin rejected; coin in records passes
...
#[test]
fn val_004_missing_coin_rejected() {
    let parent = Bytes32::from([0x04; 32]);
    let output_ph = [0x05; 32];
...
#[test]
fn val_004_coin_in_records_passes() {
    let parent = Bytes32::from([0x04; 32]);
    let output_ph = [0x05; 32];
    let cond = create_coin_condition(&output_ph, 500);
    let solution = wrap_conditions(&[cond]);

# dig-clvm: Modular CLVM Consensus Crate — Design Rationale (non-normative)

> This document is **non-normative**: it records design rationale, the upstream Chia
> crate inventory, and the reasoning behind `dig-clvm`'s orchestration approach. The
> authoritative contract — public API surface, validation semantics, invariants, error
> behavior, and conformance obligations — lives in the repository-root `SPEC.md`. Where
> the two disagree, the root `SPEC.md` is authoritative.

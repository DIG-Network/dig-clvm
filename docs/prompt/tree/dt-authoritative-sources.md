...
| IMPLEMENTATION_ORDER | `requirements/IMPLEMENTATION_ORDER.md` | Phased checklist |
...
IMPLEMENTATION_ORDER  (pick next [ ] item)
   ...
   NORMATIVE.md       (read the authoritative requirement statement)
        |
        v
   specs/PREFIX-NNN   (read the detailed specification)
        |
        v
...
Every dedicated spec contains a `Source Citations` section linking back to SPEC.md sections and/or Chia L1 source files. Follow these links to verify understanding.

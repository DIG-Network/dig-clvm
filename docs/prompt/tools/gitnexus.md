...
```
gitnexus_impact { symbol: "validate_spend_bundle" }
```

This shows every file and function that calls `validate_spend_bundle`, so you know exactly what will break if you change its signature.

### Safe rename of a type

```
gitnexus_rename { old: "ValidationResult", new: "SpendResult" }
```

Returns a list of every file and line that references `ValidationResult`, so you can update them all.
...
Shows what symbols changed since the last `analyze`. If you see changes outside the expected scope, investigate before committing.
...
- Impact analysis is most valuable for public API symbols in `src/lib.rs` and `src/consensus/mod.rs`.

...
Three tools are integrated into the dig-clvm development workflow. Each serves a distinct purpose and has mandatory usage points.
...
```
codebase_search { query: "validate spend bundle" }
```
...
# then via MCP: gitnexus_impact { symbol: "validate_spend_bundle" }
```
